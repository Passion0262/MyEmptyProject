...
object MyOffsetsUtils {
  ...
  def test(topic: String, groupId : String ):  Map[TopicPartition ,Long  ] ={
    val results: mutable.Map[TopicPartition, Long] = mutable.Map[TopicPartition ,Long ]()
    //将java的map转换成scala的map进行迭代
    import scala.collection.JavaConverters._
...
//    val offset: Map[TopicPartition, Long] = readOffset("ods_base_log", "ODS_BASE_LOG_GROUP")
//    println(offset)
    test("ods_base_log", "ODS_BASE_LOG_GROUP")
  }
